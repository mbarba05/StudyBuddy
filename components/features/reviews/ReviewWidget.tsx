...
type VoteResultMaybe =
  | { voteScore: number; myVote: -1 | 0 | 1 }
  | { vote_score: number; my_vote: -1 | 0 | 1 };
...
const ReviewWidget = ({ review, onVoted }: ReviewWidgetProps) => {
  ...
  const reviewIdForComments =
    typeof reviewIdForVotes === "number"
      ? reviewIdForVotes
      : Number(reviewIdForVotes);
  ...
  const handleCommentVote = async (commentId: number, direction: 1 | -1) => {
    ...
    setComments((cs) =>
      cs.map((c) =>
        c.id === commentId
          ? { ...c, myVote: nextMyVote, voteScore: nextScore }
          : c
      )
    );
    ...
    try {
      const raw = (await voteOnReviewComment(
        commentId,
        direction
      )) as VoteResultMaybe;
      ...
      setComments((cs) =>
        cs.map((c) =>
          c.id === commentId
            ? { ...c, myVote: res.myVote, voteScore: res.voteScore }
            : c
        )
      );
    } catch (e) {
      ...
      setComments((cs) =>
        cs.map((c) =>
          c.id === commentId
            ? { ...c, myVote: prevMyVote, voteScore: prevScore }
            : c
        )
      );
    } finally {
