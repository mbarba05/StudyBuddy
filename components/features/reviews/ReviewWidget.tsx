...
interface ReviewWidgetProps {
    ...
    onVoted?: () => Promise<void> | void; // optional: parent can refresh if review deleted
}
...
const ReviewWidget = ({ review, onVoted }: ReviewWidgetProps) => {
    ...
    // --- voting state (merged in) ---
    const [upvotes, setUpvotes] = useState(review.upvotes);
    const [downvotes, setDownvotes] = useState(review.downvotes);
    const [voteBusy, setVoteBusy] = useState(false);

    // keep local votes in sync if parent refreshes review
    useEffect(() => {
        ...
        return (
            <View className="py-2 border-b border-colors-textSecondary">
                <View className="flex-row items-center gap-2 mb-1">
                    {ppUrl ? (
                        <Image source={{ uri: ppUrl }} style={{ width: 28, height: 28, borderRadius: 14 }} />
                    ) : (
                        <View
    ...
    return (
        <View className="bg-colors-secondary w-[90vw] rounded-lg border border-colors-text p-2 gap-4 shadow-md">
            ...
            {/* bottom actions row (comments + share + voting merged) */}
            <View className="flex flex-row justify-between border-colors-textSecondary border-t pt-2">
                ...
                <View className="flex-row gap-2 items-center">
                    <TouchableOpacity onPress={() => handleVote(1)} disabled={voteBusy}>
            ...
            <Modal visible={isCommentModalOpen} transparent animationType="fade" onRequestClose={closeModal}>
                <View className="flex-1 justify-center items-center bg-black/60 px-4">
                    <View className="bg-colors-secondary border border-colors-text rounded-xl p-4 w-full gap-3">
                        ...
                        <View className="border border-colors-textSecondary rounded-lg p-2" style={{ maxHeight: 240 }}>
                            {isLoadingComments ? (
                        ...
                        <View className="flex-row justify-end gap-4">
                            ...
                            >
                                <Text className="color-colors-text text-lg">{isPosting ? "Posting..." : "Post"}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
