...
export async function getAllProfiles(opts?: {limit?: number; offset?: number }): Promise<Profile[]>{
...
export async function majorMatching(
    ...
    for(const c of candRows as any[]){
        ...
        let oc = 0; for (const x of theirCourses) if (myCourseSet.has(x)) oc++;
        let op = 0; for (const x of theirProfs) if (myProfSet.has(x)) op++;
